...
V3 的设计原则是 Runtime 不内置所有业务能力，而是把 RAG、库存、文案、推荐、交易等能力作为可注册、可替换、可审计的“外接手臂”接入。Main Agent 只负责决策、边界、审计和编排。
...
- 业务能力外接：RAG、库存、商品召回、文案生成等能力以 Tool / MCP Tool / Specialist 形式注册，并进入同一条审计链路
...
V3 不要求 Main Agent 直接实现所有业务能力。Main Agent 只做四件事：选择下一步 action、检查业务边界、记录审计 trace、编排能力调用。

具体业务能力以“外接手臂”的形式接入：

| 能力类型 | 当前实现 | 可替换方向 |
|---|---|---|
| 商品召回 | `catalog_search` 本地工具 | 搜索服务 / 推荐召回服务 |
| 库存查询 | `inventory_check` 本地工具 | 库存系统 API |
| 商品知识 RAG | `rag_product_knowledge` mock MCP tool | 外部 Modular RAG MCP Server |
| 偏好状态 | `preference_profile_update` 本地工具 | 用户偏好服务 / 实验平台 |
| 文案生成 | `marketing_copy_generate` 本地工具 | 文案生成服务 / A/B 测试平台 |
| 交易执行 | 当前不支持，HardeningGate 拦截下单 | 远期受控交易 MCP / 订单编排服务 |

这种拆法让业务能力可以独立替换、独立测试、独立审计。Runtime 层不关心 RAG 来自内置 mock server 还是外部 MCP server，只要求能力提供 schema、permission 和 observation。

## V3.1 Lite 已实现能力

当前分支没有改变 V3.0 的串行主循环，也没有把受控 fan-out runtime 伪装成已完成能力。V3.1 Lite 先证明外接能力手臂可以被统一注册、调用和审计；完整受控 fan-out runtime 留给 V3.1 正式版。
...
| `preference_profile_update` | 本地工具 | 生成可审计的显式偏好状态更新建议，不直接写 durable memory | 用户偏好服务 / 实验平台 |
